...
export async function registerRoutes(
  ...
  app.get("/api/donors", async (req, res) => {
    try {
      const { bloodType, cmvStatus, tier } = req.query;
      
      let donors = await storage.getDonors();
      
      if (bloodType && typeof bloodType === "string") {
        donors = donors.filter(d => d.bloodType === bloodType);
      }
      if (cmvStatus && typeof cmvStatus === "string") {
        donors = donors.filter(d => d.cmvStatus === cmvStatus);
      }
